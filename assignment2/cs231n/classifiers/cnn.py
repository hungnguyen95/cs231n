...
class ThreeLayerConvNet(object):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
        ...
        conv_height = (input_dim[1] + 2*pad - filter_size) // stride + 1
        conv_height = 1 + (conv_height - pool_height) // pool_stride
        conv_width = (input_dim[2] + 2*pad - filter_size) // stride + 1
        conv_width = 1 + (conv_width - pool_width) // pool_stride
        ## Weight for hidden affine layer 
        self.params['W2'] = weight_scale * np.random.randn(num_filters * conv_height * conv_width, hidden_dim)
        self.params['b2'] = np.zeros(hidden_dim)

        ## Weight for output affine layer
